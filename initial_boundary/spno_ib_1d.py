...
import SPNO_ib
# from SPNO_1d import *
importlib.reload(SPNO_ib)
from scipy.stats import multivariate_normal


x_data = np.load('spde_ib_x.npy')
y_data = np.load('spde_ib_y.npy')
...
x_train = x_data[:ntrain, ::3]
y_train = y_data[:ntrain, ::3]
x_test = x_data[-ntest:, ::3]
y_test = y_data[-ntest:, ::3]
x_grid = x_grid[::3]
...
model = SPNO_ib.SPNO1d(modes1, modes2, interpolation_dim,width, eps,x_grid.shape[-1]).cuda()
...
# torch.save(model.state_dict(),"model_ep001_spno_ib.pt")
# # np.save("data/fno_loss_1.npy",loss)
# np.save("residual_spno_ib.npy",residual)