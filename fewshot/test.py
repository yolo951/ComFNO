...
residual = np.load('residual_spno_fewshot.npy')
r_fno = np.load('residual_fno_fewshot.npy')
x_grid=np.linspace(0,1,r_fno.shape[-1])
fig, ax=plt.subplots()
plt.subplots_adjust(left=0.1, right=0.96, top=0.96, bottom=0.06)
for i in range(100):
    if not i:
        plt.plot(x_grid, r_fno[i], color='r', linestyle='dashed', zorder=1, label='FNO',linewidth=1)
        plt.plot(x_grid, residual[i], color='b', zorder=2, label='comFNO',linewidth=1)
    else:
        plt.plot(x_grid, r_fno[i], color='r', zorder=1, linestyle='dashed',linewidth=1)
        plt.plot(x_grid, residual[i], zorder=2, color='b',linewidth=1)
# plt.ylim([-0.02, 0.02])
plt.grid()
plt.legend(fontsize=14)
# plt.xlabel("$x$", fontsize=12)
# plt.ylabel('$u_p-u_g$', fontsize=12)
ax.set_xticks([0, 0.25, 0.5, 0.75, 1])
ax.set_xticklabels(['0', '', '', '', '1'], fontsize=15)
ax.set_yticks([-0.04, -0.02, 0, 0.02, 0.04])
ax.set_yticklabels(['-0.04', '', '', '', '0.04'], fontsize=15)
plt.show(block=True)
fig.savefig(r'fewshot_residual.png', dpi=300)
...
# axins = ax.inset_axes((0.30, 0.54, 0.52, 0.45))
# for i in range(100):
#     if not i:
...
# xlim0, xlim1, ylim0, ylim1 = 0, 1, -0.005, 0.005
...
#
#
# fig.savefig(r'duchou.png', dpi=300)
