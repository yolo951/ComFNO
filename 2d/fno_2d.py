...
#     plt.tight_layout()
#     plt.show(block=True)
#     fig.savefig('fno_2d_fig/fig{}.png'.format(i), dpi=300)
...
v = torch.zeros(ntest)
m = torch.zeros(ntest)
n = torch.zeros(ntest) # infinity norm
...
np.save("fno_loss_2d.npy",loss)
np.save("fno_residual_2d.npy",residual)
torch.save(model.state_dict(),'model_fno_2d.pt')
