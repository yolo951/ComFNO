...
import SPNO_tp
# from SPNO_1d import *
importlib.reload(SPNO_tp)
from scipy.stats import multivariate_normal


x_data = np.load('spde_tp_f.npy')
y_data = np.load('spde_tp_u.npy')
begin=-1
end=1
x_grid = np.linspace(begin, end, x_data.shape[-1])
...
model = SPNO_tp.SPNO1d(modes1, modes2, interpolation_dim,width, eps,x_grid.shape[-1],begin,end).cuda()
...
torch.save(model.state_dict(),"model_ep001_spno_tp.pt")
# np.save("data/fno_loss_1.npy",loss)
np.save("residual_spno_tp.npy",residual)